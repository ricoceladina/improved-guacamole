document.addEventListener("DOMContentLoaded", function () {
    ...
    playButton.addEventListener("click", function () {
        ...
        document.getElementById("gameContent").style.display = "block"; // Show game
        startGame(); // Start your game logic
    });
});
...
function startGame()
{
    ...
    // Initialize sounds
    backgroundMusic = new Audio("images/Out_Of_Time.mp3");
    backgroundMusic.loop = true;
    backgroundMusic.volume = 0.3;
    backgroundMusic.play();
    
    shootSound = new Audio("images/shotgun-firing.wav");
    shootSound.volume = 0.5;
    
    zombieHitSound = new Audio("images/shotgun-firing.wav");
    zombieHitSound.volume = 0.7;
    ...
    restartScreen = new Component(1280,720,"images/Game Over.jpg", 0,0,"image");
...
function Component(width, height, source, x, y, type, angle=0){
    ...
    this.update = function(){
        ...
        if(gameOver === false)
        {
            ...
            ctx.fillStyle = "green";
            ctx.fillRect(50, 50, 200 * (playerHealth/maxHealth), 30);
...
let showPrompt = true; // Flag to show prompt only once
...
function endGame() {
    ...
    setTimeout(() => {
        if (showPrompt) {
            ...
            if (name) {
                let added = saveScore(name, score);
                if (!added) {
                    alert("Your score did not make it to the leaderboard.");
                }
            }
...
function getLeaderboard() {
    let board = localStorage.getItem('zombieShooterLeaderboard');
...
function drawLeaderboard() {
    ...
    // Positioning the leaderboard on the lower-right side
    let leaderboardX = 925;
    let leaderboardY = 400;
    let width = 250;
    let height = 100 + (leaderboard.length * 30);

    // Draw a background box
    ctx.fillStyle = "rgba(0, 0, 0, 0.7)";
    ctx.fillRect(leaderboardX, leaderboardY, width, height);

    // Add a border
    ctx.strokeStyle = "red";
    ctx.lineWidth = 4;
    ctx.strokeRect(leaderboardX, leaderboardY, width, height);

    // Title
    ctx.fillStyle = "white";
    ctx.font = "bold 26px Comic Sans MS";
    ctx.textAlign = "center";
    ctx.fillText("LEADERBOARD", leaderboardX + width / 2, leaderboardY + 40);
    ...
    for (let i = 0; i < leaderboard.length; i++) {
        ...
        const yPos = leaderboardY + 70 + (i * 30);
        
        // Position number
        ctx.fillStyle = "gold";
        ctx.fillText(`${i + 1}.`, leaderboardX + 10, yPos);
        
        // Name
        ctx.fillStyle = "white";
        ctx.fillText(entry.name, leaderboardX + 50, yPos);
        
        // Score
        ctx.fillStyle = "lime";
        ctx.fillText(entry.score, leaderboardX + width - 50, yPos);
    }
}
...
function restartGame() {
    ...
    // Add your own game reset logic here
    startGame(); // This is your custom function to begin a new game
}
...
function Shoot(event)
{
    if(event.button === 0)
    {
        if(canShoot && !gameOver)
        {
            ...
            canShoot = false;
            shootSound.currentTime = 0; // Rewind sound if already playing
            shootSound.play();
        }
    }
}
